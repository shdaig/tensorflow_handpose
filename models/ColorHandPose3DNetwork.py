...
class ColorHandPose3DNetwork(object):
    ...
    def inference_ssd(self, image_crop, hand_side, evaluation):
        # image_crop = image.resize((256, 256), Image.ANTIALIAS)
        # image_crop = np.array(image_crop)
        # image_crop = tf.cast(image_crop, tf.float32)
        
        hand_scoremap = self.inference_detection(image_crop)
        hand_scoremap = hand_scoremap[-1]
        
        scale_crop = tf.minimum(tf.maximum(self.crop_size / 256, 0.25), 5.0)
        ...
        return hand_scoremap, image_crop, scale_crop, keypoints_scoremap, keypoint_coord3d
